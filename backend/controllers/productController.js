...
const productController = {
    ...
    /**
     * API: Thêm 1 phiên bản mới cho sản phẩm đã tồn tại (POST /api/products/:productId/variants)
     */
    addVariantToProduct: async (req, res) => {
        try {
            ...
            let rawVariant = req.body;
            if (req.body.variant !== undefined) {
                if (typeof req.body.variant === 'string') {
                    rawVariant = JSON.parse(req.body.variant);
                } else if (typeof req.body.variant === 'object' && req.body.variant !== null) {
                    rawVariant = req.body.variant;
                }
            }
            const variantErrors = validateVariantData(rawVariant);
            if (variantErrors.length > 0) {
                return res.status(400).json({ success: false, message: 'Lỗi dữ liệu phiên bản', errors: variantErrors });
            }

            const variantImages = req.files
                ? req.files.map(toStoredImageUrl).filter(Boolean)
                : [];

            const normalizedVariant = {
                sku: rawVariant.sku,
                cpu_name: rawVariant.cpu_name || null,
                cpu_benchmark_score: rawVariant.cpu_benchmark_score ? parseInt(rawVariant.cpu_benchmark_score) : null,
                gpu: rawVariant.gpu || null,
                ram_gb: parseInt(rawVariant.ram_gb),
                ram_type: rawVariant.ram_type || null,
                storage_gb: parseInt(rawVariant.storage_gb),
                color_name: rawVariant.color_name,
                original_price: parseFloat(rawVariant.original_price),
                discount_price: rawVariant.discount_price ? parseFloat(rawVariant.discount_price) : null,
                stock_quantity: rawVariant.stock_quantity ? parseInt(rawVariant.stock_quantity) : 0,
                status: rawVariant.status || 'IN_STOCK'
            };

            const newVariantId = await Product.addVariantToProduct(
                parseInt(productId),
                normalizedVariant,
                variantImages
            );
            ...
            res.status(201).json({
                ...
                data: {
                    ...
                    image_count: variantImages.length
                }
            });
        } catch (error) {
            console.error('Lỗi khi thêm phiên bản cho sản phẩm:', error);

            if (error.code === 'ER_DUP_ENTRY') {
                return res.status(409).json({
                    success: false,
                    message: 'SKU đã tồn tại. Vui lòng dùng SKU khác.'
                });
            }
            ...
            if (error instanceof SyntaxError) {
                return res.status(400).json({
                    success: false,
                    message: 'Dữ liệu variant không phải JSON hợp lệ.'
                });
            }
